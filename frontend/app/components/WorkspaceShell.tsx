...
import { BarChart3, Boxes, FileText, LayoutDashboard, LogOut, Menu, Presentation, Radar, ShieldCheck, X } from "lucide-react";
...
export function WorkspaceShell({
  ...
  return (
    <main className="min-h-screen bg-[var(--background)] text-foreground lg:h-screen lg:overflow-hidden">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-72 overflow-hidden border-r border-border bg-background/96 px-5 py-6 backdrop-blur-xl lg:flex lg:flex-col">
      ...
      <div className="lg:h-screen lg:overflow-hidden lg:pl-72">
        <header className="sticky top-0 z-40 border-b border-border bg-background/95 px-4 py-3 backdrop-blur-xl lg:hidden">
          <div className="flex min-h-12 items-center justify-between gap-3">
            <Link href="/dashboard" className="min-w-0 truncate text-lg font-bold text-foreground">
              LaunchMind AI
            </Link>
            <button
              type="button"
              onClick={() => setIsMobileMenuOpen((current) => !current)}
              className="inline-flex size-11 shrink-0 items-center justify-center rounded-lg border border-border bg-card/80 text-foreground transition hover:border-primary/35 hover:bg-primary/10"
          ...
          {isMobileMenuOpen ? (
            <div className="absolute inset-x-0 top-full border-b border-border bg-background/98 px-4 pb-4 pt-3 shadow-2xl shadow-black/40">
                ...
                {workspaceLinks.map((link) => {
                  ...
                  return (
                    <Link
                      ...
                      className={cn(
                        "flex min-h-11 items-center gap-3 rounded-lg border px-4 text-sm font-semibold transition",
                        isActive
                          ? "border-primary/35 bg-primary/12 text-foreground"
                          : "border-transparent text-muted-foreground hover:border-border hover:bg-card/70 hover:text-foreground",
                      )}
        ...
        <section className="px-4 py-7 sm:px-6 sm:py-10 lg:h-screen lg:overflow-y-auto lg:px-10 xl:px-12">
