...
export default function ReportsPage() {
  ...
  useEffect(() => {
    async function loadPageData() {
      ...
      try {
        ...
        setError("Reports could not be loaded right now. Please check your connection and try again.");
      } finally {
        setIsLoading(false);
      }
    }
  ...
  async function handleGenerateReport() {
    ...
    try {
      ...
      const report = await apiRequest<GeneratedResponse>(`/reports/generate/${selectedAnalysisId}`, {
        method: "POST",
      });
  ...
  async function handleDelete(id: number) {
    try {
      setDeletingId(id);
      await apiRequest<{ message: string }>(`/reports/${id}`, { method: "DELETE" });
        ...
        <div className="container-page">
          <div className="max-w-3xl">
            ...
            <p className="mt-5 text-base leading-7 text-[var(--muted)] sm:text-lg">
              Generate and manage formal feasibility reports from saved startup analyses.
            </p>
          </div>
          ...
          <section className="animate-fade-up hover-lift mt-10 rounded-2xl border border-[var(--border)] bg-[var(--surface-soft)] p-7 shadow-xl shadow-black/20 sm:p-8">
            <h2 className="text-2xl font-bold tracking-[-0.035em]">Generate feasibility report</h2>
            <p className="mt-3 text-sm leading-6 text-[var(--muted)]">
              Select a saved analysis and create a full university-style report.
            </p>
            ...
            {!isLoading && analyses.length === 0 ? (
              ...
              <div className="mt-6 grid gap-4 lg:grid-cols-[1fr_auto] lg:items-end">
                <div>
                  <label className="block text-sm font-semibold text-[var(--text)]" htmlFor="report-analysis">
                    Saved analysis
                  </label>
                  <select
                    id="report-analysis"
          ...
          {!isLoading && reports.length === 0 ? (
            <div className="animate-fade-up hover-lift mt-12 rounded-2xl border border-[var(--border)] bg-[var(--surface-soft)] p-9 text-center shadow-xl shadow-black/20">
              <h2 className="text-2xl font-bold tracking-[-0.035em]">No reports generated yet.</h2>
              <p className="mx-auto mt-3 max-w-xl text-sm leading-6 text-[var(--muted)]">
                Generate your first report from a saved analysis above.
              </p>
              ...
              {reports.map((report) => (
                <article
                  ...
                  className={`rounded-2xl border border-[var(--border)] bg-[var(--surface-soft)] p-7 shadow-xl shadow-black/20 transition duration-200 hover:-translate-y-0.5 hover:border-[rgba(212,175,55,0.22)] ${
                    deletingId === report.id ? "opacity-50" : "opacity-100"
                  }`}
                >
                  ...
                  <h2 className="mt-5 text-2xl font-bold tracking-[-0.035em]">{report.title}</h2>
                  <p className="mt-3 line-clamp-3 text-sm leading-6 text-[var(--muted)]">
                    {report.executive_summary}
                  </p>
                  <div className="mt-7 flex flex-col gap-3 sm:flex-row">
                    <Link href={`/dashboard/reports/${report.id}`} className="btn-secondary px-5 py-2.5">
                      Open
                    </Link>
                    <button
                      type="button"
